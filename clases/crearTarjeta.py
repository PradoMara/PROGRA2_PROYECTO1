...
def crear_tarjeta(menu, tarjetas_frame, pedido):
    ...
    num_tarjetas = len(pedido.get_children()) # O la cantidad de tarjetas ya creadas
    fila = num_tarjetas // 2
    ...
    tarjeta.bind("<Button-1>", lambda event: tarjetaClick(event, menu))
    ...
    if menu["icono_menu"]:
        ...
        imagen_label.bind("<Button-1>", lambda event: tarjetaClick(event, menu))  # Asegura que el clic en la imagen funcione
        ...
        texto_label.bind("<Button-1>", lambda event: tarjetaClick(event, menu))  # Asegura que el clic en el texto funcione
